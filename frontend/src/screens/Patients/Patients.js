...
function Patients() {
  ...
  const [patients, setPatients] = useState(memberData); // State for storing patients
  ...
  useEffect(() => {
    const fetchPatientCounts = async () => {
      try {
        ...
        setPatientCounts(response.data); // Set the patient counts from backend
      } catch (error) {
  ...
  // Define boxes with the patient statistics
  const boxes = [
    {
      id: 1,
      title: 'Today Patients',
      value: patientCounts.today,
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/patients?search=${encodeURIComponent(searchQuery)}`);
      setFilteredData(response.data); // Update state with fetched data
    } catch (error) {
  ...
  const handleRefresh = async () => {
    try {
      ...
      setPatients(response.data); // Update with latest patient data without refreshing the entire page
    } catch (error) {
      toast.error('Error refreshing patients');
    }
  };
  ...
  const addPatient = async (newPatient) => {
    try {
      ...
      setPatients((prevData) => [...prevData, response.data]); // Add new patient to the state
    ...
    <Layout>
      ...
      >
        ...
        <div className="mt-8 w-full h-96 overflow-y-auto">
          <PatientTable
            data={patients}
            setData={setPatients}  // Pass the state setter here
            functions={{
